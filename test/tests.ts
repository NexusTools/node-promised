...
const rejects = function(p: Promise<any>, reason: any, cb: (err?: Error) => void) {
  p.then((val) => {
...
const resolves = function(p: Promise<any>, equals: any, cb: (err?: Error) => void) {
  p.then((val) => {
    if(val === equals)
      return cb();
...
const rejectsIt = (msg: string, impl: () => Promise<any>, reason: any = "This is a test") =>
                                it(msg, (cb) => rejects(impl(), reason, cb));
const resolvesIt = (msg: string, impl: () => Promise<any>, equals: any = 5200) =>
  ...
  describe("With Dispose", () => {
    ...
    class Disposable {
      ...
      dispose() {
        ...
        if(i > -1) undisposed.splice(undisposed.indexOf(this), 1);
        else throw new Error("Overdisposed: " + this.stack);
      }
      static check() {
        const count = undisposed.length
        if(count) throw new Error("Not disposed enough: " + count + " remain: " + undisposed[0].stack);
      }
    }
    ...
    it("Dispose in Constructor", (cb) => {
      ...
      const promised = new Promised<Disposable>(executor, dispose);
      const promised2 = new Promised<Disposable>(null, dispose);
      const promised3 = new Promised<Disposable>(new Promise(executor), dispose);
      const promised4 = new Promised<Disposable>(new Promised(executor), dispose);
      const promised5 = new Promised<Disposable>(null, dispose);
      ...
      (promisedresolve as any).id = 1;
      (promised2resolve as any).id = 2;
      (promised3resolve as any).id = 3;
      (promised4resolve as any).id = 4;
      ...
      setImmediate(() => {
        for(let resolve of [promisedresolve, promised2resolve,
          ...
          resolve(executor);
          resolve(new Disposable("2-" + (resolve as any).id));
          resolve(new Promise(a => a(new Disposable("3-" + (resolve as any).id))));
          resolve(new Promised(new Disposable("4-" + (resolve as any).id) as any)); // Undocumented behaviour
        }
      });
